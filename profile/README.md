...
#### Payment Service
토스 페이먼츠 연동을 통한 **결제 처리와 환불 관리**를 담당하는 마이크로서비스입니다. 결제 생성부터 승인, 실패, 환불까지 **결제 라이프사이클 전반**을 책임집니다.

**핵심 책임:**
- 결제의 전체 라이프사이클 관리 (요청 → 처리 → 성공/실패/환불)
- 토스 페이먼츠 API 연동 및 결제 처리
- 결제 결과를 예매 서비스에 전송하여 상태 동기화
- 예매 취소 시 자동 환불 처리
- 상품 취소 시 관련 결제 일괄 환불
- 다건 결제·환불 지원 (여러 예매를 하나의 결제로 묶기)

**설계 핵심:**
| 특징 | 설명 |
|------|------|
| **Aggregate 중심** | Payment가 결제 상태·정책의 단일 주인 |
| **명시적 상태 머신** | PaymentStatus 기반 유효 전이만 허용 |
| **예매 동기화** | 결제 결과를 Reservation 서비스에 실시간 전달 |
| **PG사 연동** | 토스 페이먼츠 API로 결제·환불 처리 |
| **다건 처리** | 하나의 Payment에 여러 예매 연결 가능 |
...
| 모델 | 역할 |
|------|------|
| **Payment** | Aggregate Root, 결제 상태와 정책의 기준 소스 |
| **PaymentDetail** | 토스 결제키 등 PG사 결제 상세 정보 |
| **PaymentReservationLink** | 결제-예매 연결 (1:N 관계, 다건 결제 지원) |

**이벤트 연동:**
| 방향 | 이벤트 |
|------|--------|
| **발행** | 모든 결제 행위 → Log Service |

**외부 서비스 연동:**
| 서비스 | 연동 시점 | 목적 |
|--------|----------|------|
| **Reservation** | 결제 생성 시 | 예매 상태 PENDING_PAYMENT로 변경 |
| **Reservation** | 결제 완료/실패 시 | 결제 결과 전송 (상태 동기화) |
| **토스 페이먼츠** | 결제 승인 시 | PG사 결제 승인 처리 |
| **토스 페이먼츠** | 환불 시 | PG사 환불 처리 |

**동시성·정합성:**
- 결제 생성 시 예매 상태 변경 (PENDING_PAYMENT로 변경)
- 결제 승인/실패 결과 멱등성 보장 (중복 처리 방지)
- 환불 요청 멱등성 보장 (중복 환불 방지)
- 결제 결과를 예매 서비스에 전송하여 결제-예매 상태 동기화
